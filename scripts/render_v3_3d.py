#!/usr/bin/env python3
"""Render a HerdingEnv-v3 episode as a 3D animation."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Render a v3 episode in 3D")
...
def _load_model(model_type: str, model_path: str) -> Any:
...
def animate_episode(
    ...
    def _update(frame: int):
        ...
        title.set_text(
            f"Shepherding v3 3D Render  |  Step {frame}/{len(dog_history) - 1}  |  Visible sheep: {int(np.sum(visible))}/{sheep.shape[0]}"
        )
...
def main() -> None:
    ...
    max_steps = args.max_steps or int(env_cfg["max_steps"])

    env = gym.make("HerdingEnv-v3", **env_cfg)
    model = _load_model(args.model_type, args.model_path)
